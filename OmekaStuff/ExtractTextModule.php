    ...
    public function extractTextItem(Item $item, Property $textProperty, $action = 'default')
{
    ...
    $writer = new \Laminas\Log\Writer\Stream(OMEKA_PATH . '/logs/application.log');
    $log->addWriter($writer);

    $log->info("===== ExtractText: Running extractTextItem() =====");
    ...
    foreach ($itemMedia->matching($criteria) as $media) {
        if (('refresh' === $action) && ($store instanceof Local)) {
            ...
            if (!file_exists($filePath)) {
                $log->err("File not found: " . $filePath);
                continue;
            }

            // Read the actual XML content
            $xmlContent = file_get_contents($filePath);
            $log->info("Extracted raw XML content: " . substr($xmlContent, 0, 500));

            // Send XML content to be processed
            $processedText = $this->processXmlForRendering($xmlContent);

            if (!empty($processedText)) {
                $itemTexts[] = $processedText;
            }
        }
    }
    ...
    if (empty($itemText)) {
        $log->warn("Final extracted text is empty, skipping storage.");
        return;
    }

    $log->info("Final processed text before saving: " . substr($itemText, 0, 500));

    $this->setTextToTextProperty($item, $textProperty, $itemText);
}
...
private function processXmlForRendering($xmlContent)
{
    // Initialize Omeka Logger
    $log = new \Laminas\Log\Logger();
    $writer = new \Laminas\Log\Writer\Stream(OMEKA_PATH . '/logs/application.log');
    $log->addWriter($writer);
    ...
    if (empty($xmlContent)) {
        $log->warn("XML content is empty.");
        return '';
    }

    // Log the raw XML before parsing
    $log->info("Raw XML before parsing: " . substr($xmlContent, 0, 500));
    ...
    if ($xml === false) {
        $log->err("Error parsing XML: " . print_r(libxml_get_errors(), true));
        return '';
    }

    $log->info("XML loaded successfully.");

    $output = '';    // Stores the final structured text
    $columns = [];   // Stores column-based text
    $headers = [];   // Stores headers separately

    try {
        foreach ($xml->Page->TextRegion as $textRegion) {
            // Catch possible missing attributes
            if (!isset($textRegion['custom'])) {
                $log->warn("Skipping TextRegion: No 'custom' attribute found.");
                continue;
            }

            // Extract type of text region
            $attributes = (string) $textRegion['custom'];
            preg_match('/type:([a-zA-Z]+);/', $attributes, $matches);
            $type = isset($matches[1]) ? strtolower($matches[1]) : 'default';

            $log->info("Found TextRegion with type: " . $type);

            // Determine HTML wrapper based on type
            $wrapperTag = "p"; // Default to paragraph
            $class = "text-block";
            if ($type === "column") {
                $class = "column";
            } elseif ($type === "header") {
            ...
            // Extract text from TextRegion
            $regionText = '';
            foreach ($textRegion->TextLine as $line) {
                if (!isset($line->TextEquiv->Unicode)) {
                    $log->warn("Skipping line: No Unicode text found.");
                    continue;
                }
                $regionText .= '<p>' . htmlspecialchars($line->TextEquiv->Unicode) . '</p>';
            }

            // Store headers separately for layout control
            if ($type === "header") {
                $headers[] = '<div class="' . $class . '"><' . $wrapperTag . '>' . $regionText . '</' . $wrapperTag . '></div>';
            } elseif ($type === "column") {
                $columns[] = '<div class="' . $class . '">' . $regionText . '</div>';
            } else {
                $output .= '<div class="' . $class . '"><' . $wrapperTag . '>' . $regionText . '</' . $wrapperTag . '></div>';
            }
        }
    } catch (Exception $e) {
        $log->err("Error while processing XML: " . $e->getMessage());
        return '';
    }

    $log->info("Total columns found: " . count($columns));
    ...
    // Add headers if any
    if (!empty($headers)) {
        $finalOutput .= '<div class="header-container">' . implode('', $headers) . '</div>';
    }

    // Add normal text
    $finalOutput .= $output;

    // Wrap multiple columns in a flexbox container
    if (count($columns) > 1) {
        $finalOutput .= '<div class="columns-container">' . implode('', $columns) . '</div>';
    } elseif (count($columns) == 1) {
        $finalOutput .= $columns[0];
    }

    $log->info("Final processed text: " . substr($finalOutput, 0, 500));
...
{
    // Get all existing values for extracted_text
    $resourceValues = $resource->getValues();
    $criteria = Criteria::create()->where(Criteria::expr()->eq('property', $textProperty));
    
    // Remove all previous versions (both plain and HTML)
    foreach ($resourceValues->matching($criteria) as $existingValue) {
        $resourceValues->removeElement($existingValue);
    }

    // Store only the correct formatted HTML version
    if (!empty($text)) {
        $value = new Value;
        $value->setResource($resource);
        $value->setType('html'); // Ensures it's stored as HTML
